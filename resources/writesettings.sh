...
# Here we go
# Create config file
touch $CDF/resources/auto-devices/$DEVICE
# File path
FILE=$CDF/resources/auto-devices/$DEVICE
# Write some info into the device file
echo "#!/bin/bash" > $FILE
echo " " >> $FILE
echo "# Pre-configured file for $DEVICE" >> $FILE
echo "# Generated by 'auto' command" >> $FILE
...
# Prompt for data
echo " "
echo -e "$THEME$BLD - Please, enter the necessary data for this session...$WHITE"
echo " "
...
if [ "$BLDTYPE" = "K" ]; then
  BTYPE=AnyKernel
echo " "
echo -e "$THEME$BLD - Choose an option for $BTYPE Installer: "
echo " "
echo -e "$WHITE   1) Extract local $THEME$BLD$BTYPE$WHITE Template"
...
echo " "
echo -e "$THEME$BLD - Select a Kernel Source folder...$WHITE"
cd $CDF/source
select d in */; do test -n "$d" && break; echo " "; echo -e "$RED$BLD>>> Invalid Selection$WHITE"; echo " "; done
cd $CURF
echo -e "$WHITE"
...
echo -e "$THEME$BLD - Device configuration file created successfully!"
